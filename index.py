...
#TODO ADICIONAR LOGICA BAU DE CHAVES
    bausChave = positions(images['bau-chave'], threshold=ct['bau_chave'])
    bausChaveMeio = positions(images['bau-chave-50'], threshold=ct['bau_chave_50'])
    bausChaveFechado = positions(images['bau-chave-100'], threshold=ct['bau_chave_100'])
    
    global response
    if (len(bausRoxoMeio) > 0 or len(bausGoldMeio) > 0 or len(bausChaveMeio) > 0 ) and (len(bausRoxo) + len(bausGold) + len(bausBlue) + len(bausChave) + len(bausRoxoFechado) + len(bausGoldFechado) + len(bausBlueFechado)+ len(bausChaveFechado)) > 0:
        response = (
                           (len(bausRoxoMeio) + len(bausGoldMeio) + len(bausBlueMeio) + len(bausChaveMeio))
                           * 100
                   ) / (len(bausRoxo) + len(bausGold) +len(bausChave) + len(bausBlue)+len(bausRoxoFechado) + len(bausGoldFechado) + len(bausBlueFechado) +len(bausChaveFechado))
    else:
        response = 0

    global numerobau
    numerobau= (len(bausWoodMeio) + len(bausRoxoMeio) + len(bausGoldMeio) + len(bausBlueMeio)+len (bausChaveMeio) + len(bausWood)+ len(bausRoxo) + len(bausGold) + len(bausBlue)+len (bausChave) + len(bausWoodFechado)+ len(bausRoxoFechado) + len(bausGoldFechado) + len(bausBlueFechado))
    if (len(bausBlue) + len(bausBlueFechado)+ len(bausChaveFechado)) > 0 and numerobau > 10:
       response = 1
...
def NumeroBaus():
    ...
    logger('Chests Key    =>   '+'   %d +-50 hp ' % len(bausChaveMeio) + '   --   %d +-80 hp ' %len(bausChave) + '   --   %d 100 hp ' % len(bausChaveFechado))
    
    global numerobau
    numerobau= (len(bausWoodMeio) + len(bausRoxoMeio) + len(bausGoldMeio) + len(bausBlueMeio)+ len(bausWood)+ len(bausRoxo) + len(bausGold) + len(bausBlue)+ len(bausWoodFechado)+ len(bausRoxoFechado) + len(bausGoldFechado) + len(bausBlueFechado) + len(bausChaveFechado) + len(bausChaveMeio) + len(bausChave))
...
def checkChests(newMap=False):
    if(mapIsActive()):
        ...
        keys_closed = positions(images['key_chest_closed'])
        keys_hit = positions(images['key_chest_hited'])        
        ...
        keys_qtd = len(keys_hit)+len(keys_closed)
        all_chests = wood_qtd+steel_qtd+gold_qtd+diamond_qtd+jail_qtd

        result = "🟫 Baús de madeira: {}\n🟪 Baús de ferro: {}\n🟨 Baús de ouro: {}\n🟦 Baús de diamante: {}\n🏛️ Jaulas: {}\n🗝️ Keys: {}".format(wood_qtd, steel_qtd, gold_qtd, diamond_qtd, jail_qtd, keys_qtd) 
        print(result)
...
def main():
    ...
    while True:
        ...
        for last in windows:
            ...
            logger('---> window:                    %s' % last["window"].number)
